...
// Health check — for Docker/deploy readiness probes and launch.bat verification
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    service: 'captain-cool-agent',
    model: modelName,
...
app.get('/api/simulate/stream', async (req, res) => {
  ...
  try {
    ...
    // The Gemini SDK error.message is itself a JSON string — parse it for clean delivery
    let payload;
    try {
